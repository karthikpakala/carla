  ...
  for (const auto& RoadTuple : Roads) {
    ...
    for (const auto& Section : LaneSections)
    {
      ...
      for (const auto& LaneTuple : Lanes)
      {
        ...
        if (LaneType == ts::LaneType::NotSet)
          continue;
        ...
        auto AssignBoundaryType = [&](ARoadSpline* SplineActor)
        {
          switch (LaneType)
          {
            case ts::LaneType::Standard:    SplineActor->BoundaryType = ERoadSplineBoundaryType::Driving; break;
            case ts::LaneType::HovLane:     SplineActor->BoundaryType = ERoadSplineBoundaryType::Driving; break;
            case ts::LaneType::BikeLane:    SplineActor->BoundaryType = ERoadSplineBoundaryType::Biking; break;
            case ts::LaneType::NoTrucks:    SplineActor->BoundaryType = ERoadSplineBoundaryType::Driving; break;
            case ts::LaneType::Restricted:  SplineActor->BoundaryType = ERoadSplineBoundaryType::Restricted; break;
