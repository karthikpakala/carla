...
namespace road {
  ...
  void MapBuilder::CreateJunctionBoundingBoxes(Map &map) {
    for (auto &junctionpair : map._data.GetJunctions()) {
      ...
      auto waypoints = map.GetJunctionWaypoints(junction->GetId(), Lane::LaneType::NotSet);
  ...
  void MapBuilder::CheckSignalsOnRoads(Map &map) {
    for (auto& signal_pair : map._data._signals) {
      ...
      auto closest_waypoint_to_signal =
          ...
          static_cast<int32_t>(carla::road::Lane::LaneType::Standard) |  static_cast<int32_t>(carla::road::Lane::LaneType::Restricted));
      ...
      if(closest_waypoint_to_signal) {
        ...
        while(distance_to_road < (lane_width * 0.7) && iter < MaxIter && displacement_direction != 0) {
          ...
          auto right_lane_type = (right_waypoint) ? map.GetLaneType(right_waypoint.get()) : carla::road::Lane::LaneType::NotSet;

          auto left_waypoint = map.GetLeft(closest_waypoint_to_signal.get());
          auto left_lane_type = (left_waypoint) ? map.GetLaneType(left_waypoint.get()) : carla::road::Lane::LaneType::NotSet;

          if (is_rht) {
            // Move to the right if possible, then left
            if (right_lane_type != carla::road::Lane::LaneType::Standard) {
              displacement_direction = 1;
            } else if (left_lane_type != carla::road::Lane::LaneType::Standard) {
              displacement_direction = -1;
            } else {
            ...
            if (left_lane_type != carla::road::Lane::LaneType::Standard) {
              displacement_direction = -1;
            } else if (right_lane_type != carla::road::Lane::LaneType::Standard) {
              displacement_direction = 1;
            } else {
              displacement_direction = 0;
            }
